defmodule BlockScoutWeb.Tokens.Helpers do
  ...
  When the token's type is BESC-20, then we are going to format the amount according to the token's
  ...
  defp do_token_transfer_amount(%Token{type: "BESC-20"}, nil, nil, _token_ids) do
    {:ok, "--"}
  end

  defp do_token_transfer_amount(%Token{type: "BESC-20", decimals: nil}, amount, _amounts, _token_ids) do
    {:ok, CurrencyHelpers.format_according_to_decimals(amount, Decimal.new(0))}
  end

  defp do_token_transfer_amount(%Token{type: "BESC-20", decimals: decimals}, amount, _amounts, _token_ids) do
  ...
  defp do_token_transfer_amount_for_api(%Token{type: "BESC-20"}, nil, nil, _token_ids) do
    {:ok, nil}
  end

  defp do_token_transfer_amount_for_api(
         %Token{type: "BESC-20", decimals: decimals},
         amount,
         _amounts,
         _token_ids
       ) do
